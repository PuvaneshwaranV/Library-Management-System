...
const RentalTransaction = function () {
  ...
  this.init = function () {
    ...
    $("#member_name").autocomplete({
      ...
      select: (event, ui) =>
        RentalTransactionInstance.setInputFieldValuesAfterSelection(event, ui),
    ...
    $(s.table).DataTable({
      ...
      columns: [
         ...
         },
        {
          ...
          render: (d, t, row) => {
            const bgColor =
              row.bookRentalStatus === "Returned" ? "#d4edda" : "#f8d7da"; // light green / light red
            const textColor =
              row.bookRentalStatus === "Returned" ? "#155724" : "#721c24"; // dark text for contrast
